...
# Override dist2bbox to match DeepStream expected format
...
# Add DeepStream-compatible output head
...
def yolo12_export(weights, device, inplace=True, fuse=True):
    ckpt = torch.load(weights, map_location='cpu')
    ckpt = (ckpt.get('ema') or ckpt['model']).to(device).float()

    # Ensure stride and names
...
def main(args):
    ...
    print('Opening YOLO12 model')

    device = torch.device('cpu')
    model = yolo12_export(args.weights, device)
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='DeepStream YOLO12 conversion')
